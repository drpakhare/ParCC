...
def create_test_header(ws, row, test_name):
    """Create a test header with merged cells across A-F. Return next row."""
...
def add_input_row(ws, row, label, value, source_col='C', source_text=''):
    """Add an input row (label, blue input, source info). Return next row."""
...
def add_formula_row(ws, row, label, formula):
    """Add a formula row (label, green formula). Return next row."""
    ws[f'A{row}'].value = label
...
def add_parcc_output_row(ws, row, label, expected_row):
    """Add ParCC output comparison row (white input cell, match formula). Return next row."""
    ws[f'A{row}'].value = label
...
def add_blank_row(ws, row):
    """Add a blank separator row. Return next row."""
    return row + 1
...
def create_workbook():
    ...
    modules = [
        ...
        ('9. VBP', 3, ['D6', 'D16', 'D21']),
        ('10. OR-RR & Effect Size', 3, ['D6', 'D11', 'D17']),
        ('11. NNT & Log-rank HR', 3, ['D6', 'D14', 'D24']),
    ...
    row = add_input_row(ws, row, 'OR', 1.85)
    row = add_input_row(ws, row, 'Baseline risk (p0)', 0.30)
    formula_row = row
    row = add_formula_row(ws, row, 'Excel: RR = OR / (1 - p0 + p0*OR)', f'=B2/(1-B3+B3*B2)')
    row = add_parcc_output_row(ws, row, 'ParCC Output', formula_row)
    ...
    formula_row = row
    row = add_formula_row(ws, row, 'Excel: OR = RR * (1-p0) / (1 - RR*p0)', f'=B{row-2}*(1-B{row-1})/(1-B{row-2}*B{row-1})')
    row = add_parcc_output_row(ws, row, 'ParCC Output', formula_row)
    row = add_blank_row(ws, row)
    ...
    formula_row1 = row
    row = add_formula_row(ws, row, 'log(OR) = SMD * π / √3', f'=B{row-1}*PI()/SQRT(3)')
    formula_row2 = row
    row = add_formula_row(ws, row, 'OR = EXP(log(OR))', f'=EXP(B{formula_row1})')
    row = add_parcc_output_row(ws, row, 'ParCC Output', formula_row2)
    ...
    formula_row1 = row
    row = add_formula_row(ws, row, 'ARR = p_control - p_int', f'=B2-B3')
    formula_row2 = row
    row = add_formula_row(ws, row, 'NNT = CEILING(1/ARR, 1)', f'=CEILING(1/B{formula_row1},1)')
    row = add_parcc_output_row(ws, row, 'ParCC Output', formula_row2)
    row = add_blank_row(ws, row)
    ...
    row = add_input_row(ws, row, 'Baseline risk (p0)', 0.20)
    formula_row1 = row
    row = add_formula_row(ws, row, 'ARR = p0 * (1 - RR)', f'=B{row-1}*(1-B{row-2})')
    formula_row2 = row
    row = add_formula_row(ws, row, 'NNT = CEILING(1/ARR, 1)', f'=CEILING(1/B{formula_row1},1)')
    row = add_parcc_output_row(ws, row, 'ParCC Output', formula_row2)
    row = add_blank_row(ws, row)
    ...
    formula_row1 = row
    row = add_formula_row(ws, row, 'z = SQRT(Chi-square)', f'=SQRT(B{row-2})')
    formula_row2 = row
    row = add_formula_row(ws, row, 'log(HR) = z / SQRT(events/4)', f'=B{formula_row1}/SQRT(B{row-1}/4)')
    formula_row3 = row
    row = add_formula_row(ws, row, 'HR = EXP(log(HR))', f'=EXP(B{formula_row2})')
    row = add_parcc_output_row(ws, row, 'ParCC Output', formula_row3)
    ...
    formula_row1 = row
    row = add_formula_row(ws, row, 'Total observations', f'=B2+B3+B4')
    formula_row2 = row
    row = add_formula_row(ws, row, 'Expected: Stable proportion', f'=B2/B{formula_row1}')
    formula_row3 = row
    row = add_formula_row(ws, row, 'Expected: Progressed proportion', f'=B3/B{formula_row1}')
    formula_row4 = row
    row = add_formula_row(ws, row, 'Expected: Dead proportion', f'=B4/B{formula_row1}')
    formula_row5 = row
    row = add_formula_row(ws, row, 'Sum of proportions', f'=B{formula_row2}+B{formula_row3}+B{formula_row4}')
    row = add_parcc_output_row(ws, row, 'ParCC Output', formula_row5)
    row = add_blank_row(ws, row)
    ...
    formula_row1 = row
    row = add_formula_row(ws, row, 'Total observations', f'=B{row-3}+B{row-2}+B{row-1}')
    formula_row2 = row
    row = add_formula_row(ws, row, 'Expected: A proportion', f'=B{row-4}/B{formula_row1}')
    formula_row3 = row
    row = add_formula_row(ws, row, 'Expected: B proportion', f'=B{row-5}/B{formula_row1}')
    formula_row4 = row
    row = add_formula_row(ws, row, 'Expected: C proportion', f'=B{row-6}/B{formula_row1}')
    formula_row5 = row
    row = add_formula_row(ws, row, 'Sum of proportions', f'=B{formula_row2}+B{formula_row3}+B{formula_row4}')
    row = add_parcc_output_row(ws, row, 'ParCC Output', formula_row5)
    ...
    logodds1_row = row
    row = add_formula_row(ws, row, 'log-odds at t1: log((1-S)/S)', f'=LN((1-B2)/B2)')
    logodds2_row = row
    row = add_formula_row(ws, row, 'log-odds at t2: log((1-S)/S)', f'=LN((1-B4)/B4)')
    beta_row = row
    row = add_formula_row(ws, row, 'β = (logodds2 - logodds1) / (ln(t2) - ln(t1))', f'=(B{logodds2_row}-B{logodds1_row})/(LN(B5)-LN(B3))')
    lnalpha_row = row
    row = add_formula_row(ws, row, 'ln(α) = [ln(t1)*logodds2 - ln(t2)*logodds1] / (logodds2 - logodds1)', f'=(LN(B3)*B{logodds2_row}-LN(B5)*B{logodds1_row})/(B{logodds2_row}-B{logodds1_row})')
    alpha_row = row
    row = add_formula_row(ws, row, 'α = EXP(ln(α))', f'=EXP(B{lnalpha_row})')
    row = add_parcc_output_row(ws, row, 'ParCC Output', alpha_row)
    row = add_blank_row(ws, row)

    # TEST 2: Verify survival at third timepoint
    row = create_test_header(ws, row, 'TEST 2: Predict S(24) using fitted α and β')
    row = add_input_row(ws, row, 'Fitted α from TEST 1', f'=B{alpha_row}')
    row = add_input_row(ws, row, 'Fitted β from TEST 1', f'=B{beta_row}')
    row = add_input_row(ws, row, 'Prediction time t (months)', 24)
    st_row = row
    row = add_formula_row(ws, row, 'S(t) = 1 / (1 + (t/α)^β)', f'=1/(1+(B{row-1}/B{row-3})^B{row-2})')
    row = add_parcc_output_row(ws, row, 'ParCC Output', st_row)
    ...
    eligible_row = row
    row = add_formula_row(ws, row, 'N eligible = Pop * Prev * Eligible', f'=B2*B3*B4')
    adopters_row = row
    row = add_formula_row(ws, row, 'N adopters = N eligible * Uptake', f'=B{eligible_row}*B5')
    inccost_row = row
    row = add_formula_row(ws, row, 'Incremental cost per patient', f'=B7-B6')
    bi_row = row
    row = add_formula_row(ws, row, 'Budget Impact = N adopters * Inc cost', f'=B{adopters_row}*B{inccost_row}')
    row = add_parcc_output_row(ws, row, 'ParCC Output', bi_row)
    row = add_blank_row(ws, row)
    ...
    row = add_input_row(ws, row, 'New treatment cost', 22000)
    eligible_row2 = row
    row = add_formula_row(ws, row, 'N eligible = Pop * Prev * Eligible', f'=B13*B14*B15')
    adopters_row2 = row
    row = add_formula_row(ws, row, 'N adopters = N eligible * Uptake', f'=B{eligible_row2}*B16')
    inccost_row2 = row
    row = add_formula_row(ws, row, 'Incremental cost per patient', f'=B18-B17')
    undiscounted_row = row
    row = add_formula_row(ws, row, 'Undiscounted BI = N adopters * Inc cost', f'=B{adopters_row2}*B{inccost_row2}')
    discounted_row = row
    row = add_formula_row(ws, row, 'Discounted BI = Undiscounted / (1+rate)^2', f'=B{undiscounted_row}/(1+B16)^2')
    row = add_parcc_output_row(ws, row, 'ParCC Output', discounted_row)
    ...
    intl_row = row
    row = add_formula_row(ws, row, 'International$ = Cost / PPP_source', f'=B2/B3')
    ppp_row = row
    row = add_formula_row(ws, row, 'Target LCU (PPP-adjusted)', f'=B{intl_row}*B4')
    fx_row = row
    row = add_formula_row(ws, row, 'Target LCU (Market FX)', f'=B2*B5')
    ratio_row = row
    row = add_formula_row(ws, row, 'PPP/FX ratio', f'=B{ppp_row}/B{fx_row}')
    row = add_parcc_output_row(ws, row, 'ParCC Output', ratio_row)
    row = add_blank_row(ws, row)
    ...
    row = add_input_row(ws, row, 'Market FX rate (INR/USD)', 82.78)
    gdp_inr_row = row
    row = add_formula_row(ws, row, 'GDP per capita (INR)', f'=B{row-2}*B{row-1}')
    thresh1x_row = row
    row = add_formula_row(ws, row, '1x GDP threshold (INR)', f'=B{gdp_inr_row}')
    thresh3x_row = row
    row = add_formula_row(ws, row, '3x GDP threshold (INR)', f'=3*B{gdp_inr_row}')
    row = add_input_row(ws, row, 'PPP-converted cost (INR)', 2288000)
    costcheck_row = row
    row = add_formula_row(ws, row, 'Is cost > 3x threshold?', f'=IF(B{row-1}>B{thresh3x_row},"YES","NO")')
    row = add_parcc_output_row(ws, row, 'ParCC Output', costcheck_row)
    row = add_blank_row(ws, row)
    ...
    intl2_row = row
    row = add_formula_row(ws, row, 'International$ = Cost / PPP_source', f'=B{row-3}/B{row-2}')
    target_row = row
    row = add_formula_row(ws, row, 'Target LCU (INR, PPP-adjusted)', f'=B{intl2_row}*B{row-1}')
    row = add_parcc_output_row(ws, row, 'ParCC Output', target_row)
...
if __name__ == '__main__':
    print("Creating ParCC v1.4 Validation Test Cases workbook (v2)...")
    wb = create_workbook()
